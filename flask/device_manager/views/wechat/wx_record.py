    ...
    if user_id is not None:
        ...
        if len(is_user) == 1:
            ...
            res = db.session.query(Borrow_record.id, Device_information.device_name, \
                ...
                Borrow_record.num, Record_state.state).filter(Borrow_record.user_id == user_id).\
    ...
    if user_id is not None:
        ...
        if len(is_user) == 1:
            ...
            if len(is_device) == 1:
                ...
                borrow_cost = device_type.real_cost * (book_return_time - book_borrow_time).days
                if user_now.money >= borrow_cost:
                    ...
                    if device_type.available_num > num:
                        ...
                        user_now.money = user_now.money - borrow_cost
                        user_now.bor_now = user_now.bor_now + bor_now
